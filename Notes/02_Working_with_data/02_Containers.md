...
with Path('Data/prices.csv').open('rt') as f:
    for line in f:
        row = line.split(',')
...
given portfolio file and reads it into a list of **namedtuples**.  To do this,
...
Next, you must declare the named tuple at the start of your code
...
namedtuple exactly as you just did in the last exercise and append it to
this list.

Than you can append new stocks to the list:

```python
for row in rows:
    stock = Stock(name=row[0], shares=int(row[1]), price=float(row[2]))
