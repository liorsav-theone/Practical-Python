...
you have a working version of Exercise 3.17 to start.
...
```python
>>> import fileparse
>>> with Path('Data/portfolio.csv').open() as lines:
...
This change can be done using the find and replace funciton of vs code. Search for `stock\['(\w+)'\]` and replace with `stock.$1`
